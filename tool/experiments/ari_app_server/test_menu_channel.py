...
class MenuChannelTests(unittest.TestCase):
    ...
    def test_real_stdio_mcp_preserves_preview_without_claiming_persistence(self):
        context = self.channel.root / 'word.json'
        context.write_text('{}', encoding='utf-8')
        frames = [
            {'jsonrpc': '2.0', 'id': 1, 'method': 'initialize'},
            ...
            {'jsonrpc': '2.0', 'id': 2, 'method': 'tools/list'},
            {'jsonrpc': '2.0', 'id': 3, 'method': 'tools/call', 'params': {
                'name': 'execute_menu_action', 'arguments': {'id': 'import-preview', 'revision': 4, 'values': {}} }}]
        def run():
            return subprocess.run([sys.executable, str(Path(__file__).with_name('tutor_mcp.py')),
                '--context', str(context), '--menu', str(self.channel.root)],
                input='\n'.join(json.dumps(f) for f in frames)+'\n', text=True,
        ...
        with ThreadPoolExecutor() as pool:
            process = pool.submit(run)
            request = self.request()
            receipt = {'status': 'previewed', 'id': 'import-preview', 'databaseChecked': False, 'totalRows': 3, 'validRows': 2}
            self.channel.complete(request['requestId'], receipt)
            result = process.result()
        self.assertEqual(result.returncode, 0)
        responses = [json.loads(line) for line in result.stdout.splitlines()]
    ...
    def test_real_stdio_mcp_preserves_verified_deleted_status(self):
        context = self.channel.root / 'word.json'
        context.write_text('{}', encoding='utf-8')
        frames = [
            {'jsonrpc': '2.0', 'id': 1, 'method': 'initialize'},
            ...
            {'jsonrpc': '2.0', 'id': 2, 'method': 'tools/list'},
            {'jsonrpc': '2.0', 'id': 3, 'method': 'tools/call', 'params': {
                'name': 'execute_menu_action', 'arguments': {'id': 'word-delete-confirm', 'revision': 4, 'values': {}} }}]
        def run():
            return subprocess.run([sys.executable, str(Path(__file__).with_name('tutor_mcp.py')),
                '--context', str(context), '--menu', str(self.channel.root)],
                input='\n'.join(json.dumps(f) for f in frames)+'\n', text=True,
        ...
        with ThreadPoolExecutor() as pool:
            process = pool.submit(run)
            request = self.request()
            receipt = {'status': 'deleted', 'id': 'word-delete-confirm'}
            self.channel.complete(request['requestId'], receipt)
            result = process.result()
        self.assertEqual(result.returncode, 0)
        responses = [json.loads(line) for line in result.stdout.splitlines()]
