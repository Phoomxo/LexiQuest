...
function Invoke-CheckedPhase {
    ...
    Write-Host "[$($completedPhases.Count + 1)/6] $Name"
    & $Action
...
try {
    ...
    Invoke-CheckedPhase 'Formatting' {
    ...
    Invoke-CheckedPhase 'Static analysis' {
    ...
    $testTargets = @(
        ...
        'test/features/identity',
        'test/features/vocabulary',
        'test/runtime/field_feature_registry_test.dart',
        'test/runtime/app_bootstrap_test.dart',
        'test/runtime/app_build_info_test.dart',
        'test/screens/production_shell_navigation_test.dart',
        'test/screens/offline_vocabulary_journey_test.dart'
    )
    Invoke-CheckedPhase 'Focused local-first tests' {
    ...
    Invoke-CheckedPhase 'Android debug APK' {
        & flutter build apk --debug --no-pub `
            --dart-define=LEXIQUEST_VERSION=1.0.0+1 `
            --dart-define=LEXIQUEST_BUILD_ID=p1-local-first
        if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host (
        "PASS: P0-P1 local-first gate completed {0} phases in {1:mm\:ss}." -f
    ...
    $elapsed = (Get-Date) - $startedAt
    Write-Error (
        "FAIL: P0-P1 local-first gate stopped after {0} completed phases in {1:mm\:ss}. {2}" -f
        $completedPhases.Count,
        $elapsed,
        $_.Exception.Message
    )
