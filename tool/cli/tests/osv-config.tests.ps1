...
Assert-Count $text '(?m)^\[\[IgnoredVulns\]\]\s*$' 1 'exactly one ignored advisory'
Assert-Count $text '(?m)^\s*id\s*=\s*"GHSA-mh99-v99m-4gvg"\s*$' 1 'brace-expansion advisory id'
Assert-Count $text '(?m)^\s*reason\s*=\s*"\S[^"]*"\s*$' 1 'the advisory has a reason'
Assert-Count $text '(?m)^\s*ignoreUntil\s*=\s*2026-10-26\s*$' 1 'the exception expires on 2026-10-26'
