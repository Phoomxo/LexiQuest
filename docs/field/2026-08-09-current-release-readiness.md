...
| P7 physical-device checkpoint | blocked-external | After the initial zero-device ADB/PnP observation, one authorized physical Android 13 device became visible. The collector measured 7,631 MB RAM (mid tier), installed, launched, pulled, and hash-verified the exact +14 APK, and created a signed draft. Upgrade installation passed and GPU remained honest N/A, but all 13 mandatory journeys plus CPU/XNNPACK and 30-minute endurance remain pending. Distinct low- and high-tier devices are still missing. |
...
| Field evidence | same signed artifact | blocked-external | The v2 shell now contains one signed, exact-artifact mid-tier physical-device draft. Its journeys, CPU/XNNPACK benchmark, and endurance run remain pending; low/high devices and provider/cost/private/beta/rollback/owner records are absent. The verifier fails closed rather than accepting host-fake or unrelated source payloads. |
