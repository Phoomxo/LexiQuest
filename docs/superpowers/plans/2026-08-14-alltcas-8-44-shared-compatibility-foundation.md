...
Canonical new evidence always supplies its correlated Event V2, and the canonical `RecordAnswerCommand` constructor requires that event. Eventless evidence cannot be constructed through the canonical constructor; a null event is permitted only through a separately named `RecordAnswerCommand.frozenV13LegacyIngress(...)` factory whose evidence metadata exactly equals the immutable v13 migration default. That exact frozen-v13 sentinel is exclusive to the named legacy ingress and must be rejected by canonical event-bearing construction and repository writes. The persisted sentinel context therefore durably identifies eventless provenance without a new schema marker: canonical evidence cannot commit or reclassify that sentinel, and the eventless row remains eventless forever. Neither a canonical retry may omit its event nor a legacy retry may retrofit one. This task does not change the schema version, event table, generated database files, Firestore rules, or any other policy/rules artifact.

On first write, `actorIdentity` and `ownerIdentity` must both equal the attempt owner. Existing owner-upgrade authority may later remap the attempt and event `ownerIdentity` from a guest to the bound account while preserving the immutable historical guest `actorIdentity`. A committed retry after guest upgrade and database reopen must correlate against the current attempt/event `ownerIdentity`, accept the preserved historical actor, reuse the stored event with the provider unavailable, and never rewrite the event during replay.
...
- Delete the correlated event or corrupt its immutable fields and assert replay fails closed; it must not resolve the provider, synthesize a replacement, or run side effects from a candidate event. A canonical row whose correlated event is missing must remain a hard failure rather than being reclassified as frozen-v13 legacy ingress.
- Reject `event == null` for declared or ordinary canonical evidence. Accept eventless insertion only through the explicit ingress with the exact frozen-v13 legacy context; reject that exact sentinel through canonical event-bearing construction/write, and assert that replay cannot retrofit an event onto the legacy row or omit the event from a canonical row.
- Record canonical evidence as a guest, upgrade it through the existing guest-to-account owner authority, close/reopen the database, and retry as the current account while the context provider is unavailable. Assert the stored event is reused, `ownerIdentity` matches the account, the historical guest `actorIdentity` is preserved, and replay performs no event rewrite.
...
Add the event-store read needed by `replayCommittedAnswer(candidate)` and validate the stored event against the exact attempt plus the canonical whole-second timestamp and current owner lifecycle identity. Tighten the normal repository command/constructor so canonical evidence requires the correlated event and rejects the exact frozen-v13 sentinel even when an event is supplied. Isolate nullable-event compatibility behind `RecordAnswerCommand.frozenV13LegacyIngress(...)` and its exact migration-default metadata check; the sentinel itself is the durable provenance discriminator, with no new column or marker. Migrate all five inventoried direct eventless test callers—three in `test/features/sync/learning_event_sync_test.dart` and two in `test/features/progress/progress_projector_test.dart`—to that factory during this fix. Those fixture-only edits do not implement outbound/cloud sync v2, which remains owned by Task 8, and do not replace Task 7's later evidence-projection changes in the Progress tests. If an existing canonical attempt has a missing/corrupt event, fail closed; if an existing exact legacy-ingress attempt is eventless, return only its stored snapshot through that same ingress and reject any event retrofit. During committed replay, accept a historical actor that differs only because the existing owner-upgrade authority remapped the event `ownerIdentity`; never append, replace, or rewrite an event during replay. Do not change Quest event types in this task; eligibility becomes the authoritative gate in Task 7.
...
flutter test --no-pub test/features/sync/learning_event_sync_test.dart test/features/progress/progress_projector_test.dart
flutter test --no-pub test/scenarios/guest_upgrade_restart_test.dart
...
- Produces: one projection router that records immutable terminal `applied`, `notApplicable`, or `blocked` receipts and prevents forbidden side effects while preserving explicitly legacy-inferred replay.
...
Add a non-empty mutation-guard Assessment regression: seed pre-existing SRS, XP/points, achievements, reward-purchase Points rows, `rewardTransactions`, `ownedRewardItems`, and `equippedRewardItems` from eligible evidence; take byte snapshots of every named row set; record declared Enforced Assessment evidence; and assert byte-equivalence for every snapshot afterward. The regression must prove an ineligible Assessment neither deletes nor rewrites existing projections while its canonical attempt/session outcome remains durable.
...
For declared v2 evidence, the centralized validator accepts the source only when the immutable correlation is exact: `eventId == learning-event:{attemptId}`, `idempotencyKey == learning-attempt:{attemptId}:v2`, `eventVersion == 2`, and `eventType == QuizCompleted` iff the attempt is correct and `QuizAttempted` otherwise. Both event times equal the attempt occurrence canonicalized to a UTC whole second. `ownerIdentity` equals the attempt's current owner; `actorIdentity` equals that owner on first write or an immutable historical local owner whose canonical lifecycle row is inactive and `mergedInto:{currentOwner}` on replay. `aggregateType == LearningSession`, `aggregateId == attempt.sessionId`, and the unsupported envelope fields `tenantContext`, `correlationId`, `causationId`, and `providerProvenance` are all null. `privacyClassification == PrivacyClassification.anonymized`; `contentRevision` and `policyVersion` equal the persisted evidence context. The payload has exactly seven keys and no extras: `attemptId == attempt.id`, `wordId == attempt.wordId`, `promptMode == attempt.promptMode`, `correct == attempt.isCorrect`, `score == (attempt.isCorrect ? 100 : 0)`, `attemptNumber == attempt.attemptNumber`, and `evidenceContext` byte-canonicalizes to the persisted context. Reconstruct `LearningEventContext` from the envelope and require `LearningEventContext.validateAgainst` to pass for that context and exact attempt occurrence, covering consent, experiment, protocol, assignment, and feature-contract identity.

The only contextless exception is an `AnswerAttempts` row whose evidence metadata exactly equals the frozen-v13 `legacyInferred/legacy-v1/legacy` sentinel and whose source is the exact historical envelope: `eventId == learning-event:{attemptId}`, `idempotencyKey == learning-attempt:{attemptId}:v1`, `eventVersion == 1`, the same correct-to-`QuizCompleted`/incorrect-to-`QuizAttempted` mapping, canonical whole-second times, current owner plus only the authorized immutable historical-actor lineage above, `aggregateType == LearningSession`, `aggregateId == attempt.sessionId`, the same required-null unsupported fields, `privacyClassification == PrivacyClassification.anonymized`, and payload exactly `{attemptId: attempt.id}`. Its consent is exactly no-research, and `experimentContext`, `contentRevision`, and `policyVersion` are absent/null; the payload carries no evidence/research/content/policy metadata. No other missing context, approximate sentinel, v1 envelope shape, idempotency form, payload key, or metadata omission is accepted.
...
Cut reconciliation to applied projection version 2 for every source event. Before invoking a v2 sink, look for the deterministic v1 receipt `learning-projection:{projection}:{sourceEventId}:v1`. Harden that read against the correlated source: validate exact receipt ID/idempotency, owner, aggregate/causation, event version/type, canonical times, projection, applied version, outcome, and result. A v1 receipt payload must have exactly the required keys `sourceEventId`, `projection`, `appliedVersion`, `outcome`, and `result`, with values equal to the correlated source and receipt identity; only `reasonCode`, `bridgedFromVersion`, and `decision` are permitted optional keys and may be historically absent. Require event-type/outcome coherence (`LearningProjectionApplied`/`applied`, `LearningProjectionSkipped`/`notApplicable`, or `LearningProjectionBlocked`/`blocked`) and validate every optional reason that is present as a bounded stable machine reason consistent with the terminal outcome. Malformed, extra-key, contradictory, cross-owner, wrong-source, or cross-projection receipts block terminally. Quest grants used by Reward must be bounded to the same owner and source receipt rather than trusting an unscoped payload.
...
Validate the joined Quest receipt result before Reward sees it. The result object has exactly `eligible` as a boolean and `rewardGrants` as a list of at most 64 entries. An `applied` Quest result requires `eligible == true`; a `notApplicable` Quest result requires `eligible == false` and an empty grant list. Every grant object has exactly the required keys `ownerId` (String), `idempotencyKey` (String), and `xpAmount` (int), plus only the optional `rewardItemId` (String). `ownerId` must equal the source/current owner. Each idempotency key must already be trimmed, non-empty, at most 256 Unicode scalars, and unique within the receipt. XP must be positive and fit SQLite's signed 64-bit integer range (`1..9223372036854775807`). A present reward item ID must already be trimmed, non-empty, and at most 256 Unicode scalars. Add focused fail-closed regressions for a wrong source receipt, wrong grant owner, malformed result shape or key/type, duplicate or oversized idempotency key, zero/negative/out-of-range XP, non-empty grants on a skipped/not-applicable Quest, and a corrupted `applied` Quest receipt with `eligible == false`; each must block before Reward.
