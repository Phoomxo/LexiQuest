...
For declared v2 evidence, the centralized validator accepts the source only when the immutable correlation is exact: `eventId == learning-event:{attemptId}`, `idempotencyKey == learning-attempt:{attemptId}:v2`, `eventVersion == 2`, and `eventType == QuizCompleted` iff the attempt is correct and `QuizAttempted` otherwise. Both event times equal the attempt occurrence canonicalized to a UTC whole second. `ownerIdentity` equals the attempt's current owner; `actorIdentity` equals that owner on first write or an immutable historical local owner whose canonical lifecycle row is inactive and `mergedInto:{currentOwner}` on replay. `aggregateType == LearningSession`, `aggregateId == attempt.sessionId`, and the unsupported envelope fields `tenantContext`, `correlationId`, `causationId`, and `providerProvenance` are all null. `contentRevision` and `policyVersion` equal the persisted evidence context. The payload has exactly seven keys and no extras: `attemptId == attempt.id`, `wordId == attempt.wordId`, `promptMode == attempt.promptMode`, `correct == attempt.correct`, `score == (attempt.correct ? 100 : 0)`, `attemptNumber == attempt.attemptNumber`, and `evidenceContext` byte-canonicalizes to the persisted context. Reconstruct `LearningEventContext` from the envelope and require `LearningEventContext.validateAgainst` to pass for that context and exact attempt occurrence, covering consent, experiment, protocol, assignment, and feature-contract identity.

The only contextless exception is an `AnswerAttempts` row whose evidence metadata exactly equals the frozen-v13 `legacyInferred/legacy-v1/legacy` sentinel and whose source is the exact historical envelope: `eventId == learning-event:{attemptId}`, `idempotencyKey == learning-attempt:{attemptId}:v1`, `eventVersion == 1`, the same correct-to-`QuizCompleted`/incorrect-to-`QuizAttempted` mapping, canonical whole-second times, current owner plus only the authorized immutable historical-actor lineage above, `aggregateType == LearningSession`, `aggregateId == attempt.sessionId`, the same required-null unsupported fields, and payload exactly `{attemptId: attempt.id}`. Its consent is exactly no-research, and `experimentContext`, `contentRevision`, and `policyVersion` are absent/null; the payload carries no evidence/research/content/policy metadata. No other missing context, approximate sentinel, v1 envelope shape, idempotency form, payload key, or metadata omission is accepted.
