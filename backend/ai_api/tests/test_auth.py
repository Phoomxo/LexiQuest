"""Tests for the Firebase bearer-token boundary.

Mirrors ``lexiquest_voice`` tests so the two services share an identical
authentication contract. The key invariant is that token values never appear
in exceptions, detail payloads, or captured logs.
"""
...
from lexiquest_ai.auth import FirebaseTokenVerifier, extract_bearer_token
...
def test_firebase_verifier_reports_unavailable_on_certificate_fetch_error(
    ...
    with caplog.at_level("WARNING", logger="lexiquest_ai.auth"):
        with pytest.raises(HTTPException) as exc_info:
            verifier.verify(secret_token)
