"""Firebase bearer-token boundary for the LexiQuest AI API.

This module mirrors ``lexiquest_voice.auth`` exactly in shape so that both
services share an identical client/server authentication contract: a single
``TokenVerifier`` protocol, a ``FirebaseTokenVerifier`` production adapter that
never logs token contents, and an ``extract_bearer_token`` helper used by the
HTTP layer.
"""
...
from lexiquest_ai.errors import auth_unavailable
