  ...
  group('Schema v7→v8 migration — D6.1 Quest persistence', () {
    ...
    test('quest_definitions table exists after fresh onCreate', () async {
      ...
      final tables = await db
          ...
          .get();
      expect(tables, hasLength(1), reason: 'quest_definitions table must exist');
    });
    ...
    test('quest_objective_progress table exists after fresh onCreate', () async {
      await db.customSelect('SELECT 1').get();
      final tables = await db
          .customSelect(
            "SELECT name FROM sqlite_master "
    ...
    test('fresh database reports schema version 8', () async {
      await db.customSelect('SELECT 1').get();
      final version = await db
          .customSelect('PRAGMA user_version')
          .map((r) => r.read<int>('user_version'))
          .getSingle();
      expect(version, 8, reason: 'schema v8 deployed');
    });

    // ── Basic CRUD ───────────────────────────────────────────────────────────

    test('can insert and retrieve quest_definitions row', () async {
      await db.into(db.questDefinitions).insert(
    ...
    test('can insert quest_instances with FK to local_owners', () async {
      ...
      final rows = await (db.select(db.questInstances)
            ..where((t) => t.instanceId.equals('inst-01')))
          .get();
      expect(rows, hasLength(1));
    ...
    test('unique(owner_id, quest_id) prevents duplicate active instances',
        () async {
      final now = DateTime.now().toUtc();
      await db.customInsert(
        "INSERT INTO local_owners(id, account_state, created_at_utc_ms) "
      ...
      expect(
        () => db.into(db.questInstances).insert(
              QuestInstancesCompanion.insert(
    ...
    test('deleting quest_instance cascades to quest_objective_progress',
        () async {
      final now = DateTime.now().toUtc();
      await db.customInsert(
        "INSERT INTO local_owners(id, account_state, created_at_utc_ms) "
      ...
      await db.into(db.questObjectiveProgress).insert(
      ...
      await (db.delete(db.questInstances)
            ..where((t) => t.instanceId.equals('inst-cascade')))
          .go();

      final progressRows = await (db.select(db.questObjectiveProgress)
            ..where((t) => t.instanceId.equals('inst-cascade')))
          .get();
