...
void main() {
  ...
  for (final mode in EvidencePolicyRolloutMode.values) {
    testWidgets(
      'imported SRS navigation is Legacy-only under ${mode.name} rollout',
      (tester) async {
        ...
        await tester.pumpWidget(
          AppDependenciesScope(
            ...
            child: const MaterialApp(home: WordbookImportScreen()),
          ),
        );
        await tester.enterText(
          find.byType(TextField),
        ...
        await tester.tap(find.text('ประมวลผลนำเข้าคำศัพท์'));
        await tester.pumpAndSettle();
        final launch = find.widgetWithText(
          ElevatedButton,
          'เริ่มเรียนรู้คำศัพท์ชุดนี้ทันที',
        );
        tester.widget<ElevatedButton>(launch).onPressed!();
        await tester.pumpAndSettle();
...
final class _CompatibilityHarness {
  _CompatibilityHarness(EvidencePolicyRolloutMode mode)
    ...
    final owners = DriftLocalOwnerRepository(
      database,
      generateId: () => 'import-compatibility-owner',
      nowUtc: () => DateTime.utc(2026, 8, 25),
    );
    final learning = LearningUseCases(
      owners: owners,
      repository: DriftLearningRepository(database),
      generateId: () => 'import-compatibility-id',
      nowUtc: () => DateTime.utc(2026, 8, 25),
      buildInfo: const AppBuildInfo(
        version: 'test',
        buildId: 'f06-import-compatibility',
      ),
    );
