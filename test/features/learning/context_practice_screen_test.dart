...
void main() {
  ...
  testWidgets(
    'personal set opens fixed context, explains committed answer at 320px text200',
    (tester) async {
      ...
      try {
        ...
        await settle();
        final start = find.textContaining('ฝึกบริบท · เลือกคำที่สับสน');
        await tester.ensureVisible(start);
        await tester.tap(start);
        await settle();
        expect(
          find.byKey(const ValueKey('cloze-prompt')),
          findsOneWidget,
          reason: find
              .byType(Text)
              .evaluate()
        ...
        expect(find.byKey(const ValueKey('cloze-mode-typed')), findsNothing);
        final wrong = find.byKey(
          const ValueKey('cloze-option-word:starter-book-pencil'),
        );
        ...
        final why = find.text('เหตุผลในบริบทนี้');
        await tester.ensureVisible(why);
        await settle();
        expect(why, findsOneWidget);
        expect(tester.takeException(), isNull);
        await captureR15Surface(tester, 'e32-context-why-320-text200');
