...
import 'speaking_capture_test.dart' show Gateway;
...
void main() {
  ...
  for (final mode in ['spoken', 'fallback', 'retiredReady']) {
    final fallback = mode == 'fallback';
    testWidgets(
      'personal set opens mode=$mode and durable rubric at 320px text200',
      (tester) async {
        ...
        final dependencies = AppDependencies(
          ...
          learning: learning,
          personalSets: sets,
          speakingScenarios: SpeakingScenarioUseCases(
            sets: sets,
            isAvailable: () => enabled,
          ),
        ...
        try {
          ...
          await settle();
          final start = find.text('Speak in a scenario · ฝึกพูด');
          await tester.ensureVisible(start);
          await tester.tap(start);
          await settle();
          expect(find.text('Speak in a scenario'), findsOneWidget);
          ...
          if (fallback) {
            ...
            await settle();
            final confirm = find.byKey(const Key('speaking-confirm'));
            await tester.ensureVisible(confirm);
            await tester.tap(confirm);
            await settle();
          }
          final submit = find.byKey(const Key('speaking-submit'));
          ...
          expect(
            find.textContaining(
              fallback
                  ...
                  : 'Confirmed spoken transcript',
            ),
            findsOneWidget,
          );
          final rows = await tester.runAsync(
            () => db.select(db.speakingPracticeResults).get(),
          );
          expect(rows, hasLength(1));
          expect(tester.takeException(), isNull);
          await captureR15Surface(
            tester,
            'e41-speaking-fallback-$fallback-result-320-text200',
          );
