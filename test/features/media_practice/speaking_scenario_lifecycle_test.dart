...
void main() {
  ...
  SpeakingScenarioUseCases app() =>
      SpeakingScenarioUseCases(sets: sets, isAvailable: () => enabled);
  Future<SpeakingScenarioTicket> open() async => app().open(
    await sets.begin(),
    setId: 'set',
    setRevision: 1,
    memberIndex: 0,
    activityId: 'speaking',
    intent: SpeakingIntent.assessment,
  );
  test('intent is pinned and six turns bound a scenario', () async {
    final service = app();
    var ticket = await open();
    for (var i = 0; i < 6; i++) {
    ...
    expect(ticket.results, hasLength(6));
    await expectLater(
      service.submit(
        ticket,
        operationId: 'seventh',
  ...
  test(
    'unconfirmed ASR and text fallback remain distinct through restart',
    () async {
      final service = app();
      final pending = await service.submit(
        await open(),
        operationId: 'uncertain',
        response: 'I drink a book.',
      );
      expect(pending.results.single.scores, isNull);
      final typed = await service.submit(
        pending,
        operationId: 'typed',
        response: 'i read a book',
        input: SpeakingInput.textFallback,
      );
      expect(typed.results.last.input, SpeakingInput.textFallback);
      await db.close();
      db = AppDatabase(NativeDatabase(databaseFile));
      await wire();
      final saved = await open();
      expect(saved.results.first.status, RubricStatus.uncertain);
      expect(saved.results.first.confirmed, isFalse);
      expect(saved.results.last.input, SpeakingInput.textFallback);
      expect(saved.results.last.scores, [2, 2, 2]);
    },
  );
  test(
  ...
  test(
    ...
    () async {
      ...
      final payload = (await db.select(db.speakingPracticeResults).get())
          .single
          .payloadJson;
      await db.customStatement(
      ...
      final row = (await db.select(db.speakingPracticeResults).get()).single;
      ...
      expect(utf8.decode(archive.bytes), contains('speakingPracticeResults'));
  ...
  test(
    ...
    () async {
      ...
      await db.delete(db.speakingPracticeResults).go();
  ...
  test('immutable rows reject replacement and direct rubric mutation', () async {
    ...
    await expectLater(
      db.customStatement(
        "UPDATE speaking_practice_results SET payload_json='{}'",
      ),
      throwsA(anything),
    );
    await expectLater(
      db.customStatement(
        'INSERT OR REPLACE INTO speaking_practice_results SELECT * FROM speaking_practice_results',
      ),
      throwsA(anything),
    );
  ...
  test(
    ...
    () async {
      ...
      duringAdmission = () async {
        if ((await db.select(db.speakingPracticeResults).get()).isNotEmpty) {
          sawPendingRow = true;
          ticket.cancel();
        }
      };
