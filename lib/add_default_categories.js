...
// กำหนดเส้นทางไฟล์ให้ตรงกับตำแหน่งจริงของไฟล์ JSON
admin.initializeApp({
  credential: admin.credential.cert(path.join('C:/Users/Natapong/Downloads/vocab-learning-app-219ef-firebase-adminsdk-yjmlk-ddd511027d.json')),
  databaseURL: 'https://vocab-learning-app-219ef.firebaseio.com',
});
...
const addDefaultCategories = async () => {
  for (const category of defaultCategories) {
    const categoryRef = db.collection('categories').doc(category.category_name); // ใช้ category_name แทน name
    const categoryData = {
      name: category.category_name, // เปลี่ยนจาก category.name เป็น category.category_name
      words: category.words, // เก็บคำศัพท์จาก default categories
      created_at: admin.firestore.FieldValue.serverTimestamp(),
    };

    try {
      await categoryRef.set(categoryData);
      console.log(`Added category to categories collection: ${category.category_name}`); // ใช้ category.category_name
    } catch (error) {
      console.error(`Error adding category to categories: ${category.category_name}`, error); // ใช้ category.category_name
    }
  }
};


// เรียกฟังก์ชันเพิ่มหมวดหมู่เริ่มต้น
